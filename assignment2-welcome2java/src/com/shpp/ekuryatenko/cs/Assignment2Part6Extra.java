...
/*
 * File: Assignment2Part6Extra.java
 *
 * Caterpillar and RandomGenerator
 *
 */
import acm.graphics.*;
...
public class Assignment2Part6Extra extends WindowProgram {
    ...
    * - OFFSET 2 - distance between pairs of ovals.*/
    private static final double MAX_OFFSET_1_dX = WHOLE_OVAL_DIAM * 0.65;
    private static final double MAX_OFFSET_1_dY = WHOLE_OVAL_DIAM * 0.65;
    private static final double MAX_OFFSET_2_dX = WHOLE_OVAL_DIAM * 0.5;
    private static final double MAX_OFFSET_2_dY = -0.9 * MAX_OFFSET_1_dY;

    /* Adjusting position of first oval in caterpillar*/
    private static final double START_X = 0;
    private static final double START_Y = MAX_OFFSET_1_dY;
     ...
     * - Y Distances between ovals gets randomly
    ...
    private double getNew_X(int iteration, double lastPosition_X) {
        if (iteration % 2 == 0) {
            return lastPosition_X + MAX_OFFSET_2_dX;
        } else {
            return lastPosition_X + MAX_OFFSET_1_dX;
        }
    }
    ...
    private double getNew_Y(int iteration, double lastPosition_Y) {
        if (iteration % 2 == 0) {
            return lastPosition_Y - MAX_OFFSET_2_dY * randomKoef();
        } else {
            return lastPosition_Y - MAX_OFFSET_1_dY * randomKoef();
        }
    }

    private double  randomKoef() {
        RandomGenerator rgen = new RandomGenerator();

        return roundDouble(rgen.nextDouble(0.2,0.8));
    }

    /* Double round method   */
    private double roundDouble(double myDouble) {
        double buf = myDouble * 10;
